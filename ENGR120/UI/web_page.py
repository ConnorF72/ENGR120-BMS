...
<body>
  ...
  <!-- Status cards — values are updated by JS fetch, not page reload -->
  ...
  <!-- Controls — these are never reloaded by JS, only by explicit user action -->
  <div class="controls">
    <h2>Controls</h2>
  ...
  <script>
    // -------------------------------------------------------------------
    // Fetches /?status=1 every 3 seconds and updates only the status cards
    // The control forms are part of the same HTML page and are never touched
    // -------------------------------------------------------------------
    ...
    function updateCards(data) {{
      ...
      // Kill switch button/row reflect live state
